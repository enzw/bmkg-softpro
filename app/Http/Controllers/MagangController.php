...
class MagangController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($request->hasFile('kartu_identitas') && in_array($jenis_layanan, ['Layanan Kunjungan Teknis'])) {
            try {
                ...
                $fileName = str_replace([':', ' ', '(', ')'], '_', $fileName);
                $path = $file->storeAs($directory, $fileName, 's3'); // Store to S3/R2 similarly
    ...
    {
        try {
            // Search record in all service tables
            $tables = [
                Magang::class,
                Asuransi::class,
                LayananData::class,
            ...
            foreach ($tables as $model) {
                ...
                if ($found) {
                    $record = $found;
                    if ($found->surat_permohonan && str_contains($found->surat_permohonan, $fileName)) {
                        $filePath = $found->surat_permohonan;
                    } elseif (isset($found->ktp) && $found->ktp && str_contains($found->ktp, $fileName)) {
                        $filePath = $found->ktp;
                    } elseif (isset($found->kartu_identitas) && $found->kartu_identitas && str_contains($found->kartu_identitas, $fileName)) {
                        $filePath = $found->kartu_identitas;
                    } elseif (isset($found->kartu_mahasiswa) && $found->kartu_mahasiswa && str_contains($found->kartu_mahasiswa, $fileName)) {
            ...
            if (!$record || !$filePath) {
                \Log::warning("File matching [{$fileName}] not found in database record for ID [{$id}]");
                abort(404, 'File tidak ditemukan');
            }

            // Check permission: only uploader or admin
            $user = Auth::user();
            $isAdmin = $user && ($user->role === 'admin' || $user->role === 'superuser' || $user->role === 'superuser');
            $isOwner = $record->user_id === $user?->id;
            ...
            \Log::error('Gagal download file: ' . $error->getMessage());
    ...
    public function downloadFileSimple($fileName)
    {
        try {
            ...
            foreach ($models as $model) {
                $found = $model::where(function ($query) use ($fileName) {
            ...
            if (!$record || !$filePath) {
                \Log::warning("No database record found matching filename [{$fileName}] across service tables");
                abort(404, 'File tidak ditemukan di database.');
            }

            // Authorize
            $user = Auth::user();
            if ($user->role !== 'admin' && $user->role !== 'superuser' && $record->user_id !== $user->id) {
            ...
            \Log::error('Error downloading service file: ' . $e->getMessage(), [
                'user_id' => Auth::id(),
                'fileName' => $fileName
            ]);
