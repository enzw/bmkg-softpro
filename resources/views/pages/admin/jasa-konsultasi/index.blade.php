...
@section('content')
                    ...
                    <div class="flex items-center justify-between">
                        <div>
                            <h1 class="text-4xl font-bold text-gray-900 dark:text-white mb-2">
                                Data Jasa Konsultasi
                            </h1>
                            <p class="text-gray-600 dark:text-gray-400">
                                Kelola semua permohonan jasa konsultasi di sini.
                            </p>
                        </div>
                        <a href="{{ route('admin.jasa-konsultasi.create') }}"
                ...
                @if ($jasa_konsultasi->count() > 0)
                <div class="space-y-4">
                    @foreach ($jasa_konsultasi as $item)
                        <div class="bg-white dark:bg-gray-800 rounded-xl p-6 border border-gray-100 dark:border-gray-700 hover:shadow-md dark:hover:shadow-lg transition group">
                            ...
                            <div class="flex gap-3">
                                <a href="{{ route('admin.jasa-konsultasi.edit', $item->id) }}"
                                ...
                                <div id="modal-delete-{{ $loop->index }}" class="hidden fixed inset-0 z-50 overflow-auto bg-black/40 backdrop-blur-sm flex items-center justify-center p-4 transition-opacity duration-300">
                                    <div class="bg-white dark:bg-gray-800 rounded-3xl max-w-sm w-full shadow-2xl transform transition-all duration-300 scale-95 hover:scale-100">
                                        ...
                                        <div class="px-8 pt-6 pb-8 text-center">
                                            <h3 class="text-2xl font-bold text-gray-900 dark:text-white mb-2">Hapus Jasa Konsultasi?</h3>
                                            <p class="text-gray-600 dark:text-gray-400 text-sm leading-relaxed">
                                                Data jasa konsultasi <strong class="text-gray-900 dark:text-white">{{ $item->nama }}</strong> akan dihapus secara permanen.
                                            </p>
                                        </div>
                ...
                <div class="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-6 text-center">
                    <p class="text-gray-600 dark:text-gray-400">Belum ada permohonan jasa konsultasi.</p>
                </div>
                @endif
...
function deleteRecord(recordId, modalId) {
    ...
    fetch(`/admin/jasa-konsultasi/${recordId}`, {
        method: 'DELETE',
        headers: {
    ...
    .then(result => {
        if (result.ok) {
            ...
            const errorMsg = result.data && result.data.message ? String(result.data.message) : 'Gagal menghapus jasa konsultasi';
            alert(errorMsg);
        }
    })
    .catch(error => {
