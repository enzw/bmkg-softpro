<div class="bg-white dark:bg-gray-800 rounded-[2.5rem] border border-gray-100 dark:border-gray-700 shadow-sm overflow-hidden">
    ...
    <div class="p-8 border-b border-gray-50 dark:border-gray-700 flex items-center justify-between bg-gray-50/30 dark:bg-gray-900/10">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h2 class="text-lg font-bold text-gray-900 dark:text-white uppercase tracking-tight">Formulir Kunjungan</h2>
                <p class="text-[10px] text-gray-400 font-semibold uppercase tracking-widest text-shadow-sm">Lengkapi data kunjungan teknis</p>
            </div>
        </div>
    ...
    <form
        ...
        <div class="relative pl-6 border-l-2 border-green-500/30 space-y-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div>
                    <label for="jenis_kunjungan" class="block text-sm font-medium text-gray-900 dark:text-white mb-2">
                    ...
                    <select name="jenis_kunjungan" id="jenis_kunjungan" required
                        ...
                        <option value="goes to BMKG" @selected($is_edit && $permohonan ? old('jenis_kunjungan', $permohonan->jenis_kunjungan) == 'goes to BMKG' : old('jenis_kunjungan') == 'goes to BMKG')>Datang
                            ke BMKG</option>
        ...
        <div class="relative pl-6 border-l-2 border-green-500/30 space-y-6">
            ...
            <div>
                ...
                <textarea name="rencana_kunjungan" id="rencana_kunjungan" rows="4" placeholder="Jelaskan rencana kunjungan Anda..." required
                    ...
                    <select name="status" id="status" required
                        ...
                        @foreach ($statuses as $status)
                            <option value="{{ $status->value }}"
                                @selected(old('status', isset($permohonan->status) ? $permohonan->status->value : '') == $status->value)>
                                {{ $status->label() }}
                            </option>
                        @endforeach
                    </select>
