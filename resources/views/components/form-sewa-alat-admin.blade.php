...
<div
    class="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-100 dark:border-gray-700 p-8 h-max lg:sticky lg:top-20">
    <!-- Header -->
    <div class="mb-8">
        <div class="flex items-center gap-3 mb-3">
            ...
            <h2 class="text-2xl font-bold text-gray-900 dark:text-white">
                {{ $is_edit ? 'Edit Permohonan' : 'Buat Permohonan' }}</h2>
        </div>
    ...
    <form action="{{ $is_edit ? route('admin.sewa-alat.update', $permohonan->id) : route('admin.sewa-alat.store') }}"
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
            <div>
                ...
                <input id="nama" type="text" name="nama" placeholder="Masukkan nama lengkap"
                    value="{{ $is_edit ? old('nama', $permohonan->nama) : old('nama') }}" required
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input id="banyak_unit" type="number" name="banyak_unit"
                    value="{{ $is_edit ? old('banyak_unit', $permohonan->banyak_unit) : old('banyak_unit', 1) }}"
                    required
                    class="w-full px-4 py-3 rounded-lg border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-900 text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent dark:focus:ring-green-400 transition" />
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
            <div>
                ...
                <input id="sewa_mulai" type="date" name="sewa_mulai"
                    value="{{ $is_edit ? ($permohonan->sewa_mulai ? $permohonan->sewa_mulai->format('Y-m-d') : '') : old('sewa_mulai') }}"
                    required
            ...
            <div>
                ...
                <input id="sewa_berakhir" type="date" name="sewa_berakhir"
                    value="{{ $is_edit ? ($permohonan->sewa_berakhir ? $permohonan->sewa_berakhir->format('Y-m-d') : '') : old('sewa_berakhir') }}"
                    required
                    class="w-full px-4 py-3 rounded-lg border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-900 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent dark:focus:ring-green-400 transition" />
        ...
        @if ($is_edit)
            <div>
                ...
                <select name="status" id="status" required
                    ...
                    @foreach (['Belum Lunas', 'Siap Diambil', 'Dibawa', 'Dikembalikan', 'Ditolak'] as $option)
                        <option value="{{ $option }}" @selected(old('status', $permohonan->status ?? '') == $option)>
                            {{ $option }}
                        </option>
                    @endforeach
                </select>
