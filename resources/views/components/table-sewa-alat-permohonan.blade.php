                        ...
                        <div class="bg-white dark:bg-gray-800 rounded-2xl max-w-md w-full shadow-2xl overflow-hidden">
                                ...
                                <div class="flex items-center justify-between">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <h3 class="text-lg font-bold">{{ e($item->alat->nama ?? '-') }}</h3>
                                        </div>
                                    </div>
                            ...
                            <div class="p-6 space-y-6 max-h-[calc(100vh-240px)] overflow-y-auto">
                                ...
                                <div class="flex items-center justify-between">
                                    ...
                                    @php
                                        $statusValue = $item->status instanceof \App\Enums\SewaStatus ? $item->status->value : (string)$item->status;
                                        $statusLabel = $item->status instanceof \App\Enums\SewaStatus ? $item->status->label() : $statusValue;
                                        $statusColor = match($statusValue) {
                                            'Belum Lunas' => 'bg-yellow-100 dark:bg-yellow-900/30 text-yellow-700 dark:text-yellow-300 border border-yellow-200 dark:border-yellow-800',
                                            'Siap Diambil' => 'bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 border border-blue-200 dark:border-blue-800',
                                            'Dibawa' => 'bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-300 border border-purple-200 dark:border-purple-800',
                                            'Dikembalikan' => 'bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300 border border-green-200 dark:border-green-800',
                                ...
                                <!-- Informasi Penyewa -->
                                <div class="space-y-3">
                                    <h4 class="text-xs font-bold text-gray-600 dark:text-gray-300 uppercase tracking-widest">Informasi Penyewa</h4>
                                    <div class="grid grid-cols-2 gap-3">
                                        <div class="bg-gray-50 dark:bg-gray-700/50 p-3 rounded-lg">
                                            <p class="text-xs text-gray-500 dark:text-gray-400 font-semibold mb-1">Nama</p>
                                            <p class="text-sm font-semibold text-gray-900 dark:text-white">{{ e($item->nama) }}</p>
                                        </div>
                                        <div class="bg-gray-50 dark:bg-gray-700/50 p-3 rounded-lg">
                                ...
                                <!-- Informasi Sewa -->
                                <div class="space-y-3">
                                    <h4 class="text-xs font-bold text-gray-600 dark:text-gray-300 uppercase tracking-widest">Periode Sewa</h4>
                                    <div class="grid grid-cols-2 gap-3">
                                        <div class="bg-gray-50 dark:bg-gray-700/50 p-3 rounded-lg">
                                            <p class="text-xs text-gray-500 dark:text-gray-400 font-semibold mb-1">Mulai</p>
                                            <p class="text-sm font-semibold text-gray-900 dark:text-white">{{ \Carbon\Carbon::parse($item->sewa_mulai)->format('d/m/Y') }}</p>
                                        </div>
                                        <div class="bg-gray-50 dark:bg-gray-700/50 p-3 rounded-lg">
                                            <p class="text-xs text-gray-500 dark:text-gray-400 font-semibold mb-1">Berakhir</p>
                                            <p class="text-sm font-semibold text-gray-900 dark:text-white">{{ \Carbon\Carbon::parse($item->sewa_berakhir)->format('d/m/Y') }}</p>
                                        </div>
                                    </div>
                                </div>
                                ...
                                @if($item->keterangan)
                                    ...
                                    <div class="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg border border-blue-200 dark:border-blue-800">
                                        <p class="text-xs font-bold text-blue-600 dark:text-blue-400 uppercase tracking-widest mb-2">Keterangan</p>
                                        <p class="text-sm text-blue-900 dark:text-blue-100 whitespace-pre-wrap">{{ $item->keterangan }}</p>
                                    </div>
                                @endif
                                ...
                                @if($item->expedisi && $item->resi)
                                    <!-- Pengiriman -->
                                    <div class="space-y-3">
                                        <h4 class="text-xs font-bold text-gray-600 dark:text-gray-300 uppercase tracking-widest">Informasi Pengiriman</h4>
                                        <div class="grid grid-cols-2 gap-3">
                                            <div class="bg-gray-50 dark:bg-gray-700/50 p-3 rounded-lg">
                                                <p class="text-xs text-gray-500 dark:text-gray-400 font-semibold mb-1">Kurir</p>
                                                <p class="text-sm font-semibold text-gray-900 dark:text-white">{{ e($item->expedisi) }}</p>
                                            </div>
                                            <div class="bg-gray-50 dark:bg-gray-700/50 p-3 rounded-lg">
                                                <p class="text-xs text-gray-500 dark:text-gray-400 font-semibold mb-1">No. Resi</p>
                                                <p class="text-sm font-semibold text-gray-900 dark:text-white">{{ e($item->resi) }}</p>
                                            </div>
                                        </div>
                                    </div>
                                @endif
                                ...
                                @if($item->surat_permohonan || $item->ktp)
                                    ...
                                    <div class="space-y-2 pt-4 border-t border-gray-200 dark:border-gray-700">
                                        @if($item->surat_permohonan)
                                            <a href="{{ route('permohonan.sewa-alat.download', ['fileName' => basename($item->surat_permohonan)]) }}"
                                        ...
                                        @endif
                                        @if($item->ktp)
                                            <a href="{{ route('permohonan.sewa-alat.download', ['fileName' => basename($item->ktp)]) }}"
                            ...
                            <div class="p-6 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/50">
                                <button type="button" onclick="closeModal('modal-{{ $loop->index }}')"
                                    class="w-full px-4 py-3 rounded-lg bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white font-semibold transition shadow-lg">
                                    Tutup
                                </button>
                            </div>
                        </div>
